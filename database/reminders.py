# reminders.py
"""Provides access to the tables "reminders" in the database"""
...
@dataclass()
class Reminder():
    """Object that represents a record from the table "reminders"."""
    activity: str
    channel_id: int
    clan_name: str
    custom_id: int
    ...
    async def refresh(self) -> None:
        ...
        """
        try:
            new_settings = await get_reminder(self.user_id, self.activity, self.custom_id)
...
async def _dict_to_reminder(record: dict) -> Reminder:
    ...
    Arguments
    ---------
    record: Database record from table "reminders" as a dict.
...
# Read Data
async def get_reminder(user_id: int, activity: str, custom_id: Optional[int] = None) -> Reminder:
...
async def get_active_reminders(user_id: Optional[int] = None, activity: Optional[str] = None,
    ...
    """
    table = 'reminders'
    function_name = 'get_active_reminders'
...
async def get_due_reminders(user_id: Optional[int] = None) -> Tuple[Reminder]:
    ...
    """
    table = 'reminders'
    function_name = 'get_due_reminders'
    if user_id is None:
...
async def get_old_reminders(user_id: Optional[int] = None) -> Tuple[Reminder]:
    ...
    """
    table = 'reminders'
    function_name = 'get_old_reminders'
    if user_id is None:
        sql = f'SELECT * FROM {table} WHERE end_time < ?'
    else:
...
async def insert_reminder(user_id: int, activity: str, time_left: timedelta,
    ...
    if activity != 'custom':
        try:
            reminder = await get_reminder(user_id, activity)
        except exceptions.NoDataFoundError:
            pass
    if reminder is not None:
        ...
        try:
            ...
            raise
        reminder = await get_reminder(user_id, activity, custom_id)
