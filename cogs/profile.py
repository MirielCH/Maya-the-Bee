...
class ProfileCog(commands.Cog):
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        if message.embeds:
            ...
            embed_field_statistics = embed_field_production_rate = embed_field_production_state = ''
            embed_field_tool_status = ''
            if embed.author:
                embed_author = embed.author.name
                icon_url = embed.author.icon_url
            embed_description = embed.description if embed.description else ''
            if len(embed.fields) >= 3:
                embed_field_statistics = embed.fields[0].value
                embed_field_production_rate = embed.fields[1].value
                embed_field_production_state = embed.fields[2].value
                if len(embed.fields) > 3:
                    embed_field_tool_status = embed.fields[3].value
            ...
            if any(search_string in embed_author.lower() for search_string in search_strings):
                ...
                if embed_field_tool_status != '':
                    cooldowns = []
                    ready_commands = []
                    if user_settings.reminder_research.enabled:
                        timestring_match = re.search(r"researching: `(.+?)` remaining", embed_field_tool_status.lower())
                        if timestring_match:
                    ...
                    if user_settings.reminder_upgrade.enabled:
                        timestring_match = re.search(r"upgrading: `(.+?)` remaining", embed_field_tool_status.lower())
                        if timestring_match:
