...
class CooldownsCog(commands.Cog):
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        if message.embeds:
            ...
            embed_field_commands = embed_field_quests = embed_field_raid = embed_field_tool = ''
            if embed.author:
                embed_author = embed.author.name
                icon_url = embed.author.icon_url
            embed_title = embed.title if embed.title else ''
            if len(embed.fields) >= 4:
                embed_field_commands = unicodedata.normalize('NFKD', embed.fields[0].value)
                embed_field_quests = unicodedata.normalize('NFKD', embed.fields[1].value)
                embed_field_raid = unicodedata.normalize('NFKD', embed.fields[2].value)
                embed_field_tool = unicodedata.normalize('NFKD', embed.fields[3].value)
            ...
            if any(search_string in embed_title.lower() for search_string in search_strings):
                ...
                try:
                    ...
                    return
                if not user_settings.bot_enabled: return
                timestring_match = re.search(r'in \*\*`(.+?)`\*\*$', embed_title.lower())
