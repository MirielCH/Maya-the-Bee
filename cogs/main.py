...
class MainCog(commands.Cog):
    ...
    async def on_application_command_error(self, ctx: discord.ApplicationContext, error: Exception) -> None:
        ...
        command_name = await functions.get_maya_slash_command(self.bot, command_name)
        ...
        if isinstance(error, commands.NoPrivateMessage):
            ...
            await ctx.respond(
                f'Bzzt! **{ctx.author.global_name}**, looks like I don\'t know you yet.\n'
                f'Use {await functions.get_maya_slash_command(self.bot, "on")} to activate me first.',
                ephemeral=True
            )
    ...
    async def on_command_error(self, ctx: commands.Context, error: Exception) -> None:
        ...
        if isinstance(error, (commands.CommandNotFound, commands.NotOwner)):
            ...
            await ctx.reply(
                f'**{ctx.author.global_name}**, you can only use this command every '
            ...
            await ctx.reply(
                f'**{ctx.author.global_name}**, looks like I don\'t know you yet.\n'
                f'Use {await functions.get_maya_slash_command(self.bot, "on")} to activate me first.',
            )
        elif isinstance(error, (commands.UnexpectedQuoteError, commands.InvalidEndOfQuotedStringError,
                                commands.ExpectedClosingQuoteError)):
            await ctx.reply(
                f'**{ctx.author.global_name}**, whatever you just entered contained invalid characters I can\'t process.\n'
    ...
    async def on_ready(self) -> None:
        ...
        await self.bot.change_presence(activity=discord.Activity(type=discord.ActivityType.playing,
                                                                 name='Are you in Pro League 3 yet?'))
    @commands.Cog.listener()
